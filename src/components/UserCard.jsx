...
export default function UserInfoCard({ name, role, phone, id, image, className }) {
  return (
    <div
      dir="rtl"
      className={`h-[275px] bg-white/90 backdrop-blur-md border border-gray-200 rounded-2xl shadow-md p-6 w-full  flex flex-col items-center text-center transition-transform hover:-translate-y-1 hover:shadow-lg ${className}`}
    >
      <div className="w-16 h-16 rounded-full overflow-hidden border-4 border-indigo-100 shadow-sm mb-4">
        <img
          src={image}
          alt={`${name}'s profile`}
